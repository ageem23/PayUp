...
| 2026-06-18 | 1.4.0 | CodeRabbit re-review (1 finding): `applySession` only enforced the whitelist when `email` was truthy, so a session with a missing/empty email was accepted (fail-open). Hardened to **fail closed** — any session lacking a whitelisted email is signed out. Lint + build clean. | Amelia (Dev) |
| 2026-06-18 | 1.5.0 | CodeRabbit re-review of the fail-closed fix came back clean (no new actionable feedback). Review loop converged. Status → done (ready-to-merge). | Amelia (Dev) |
